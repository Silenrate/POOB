...
public class CityOfHeroesContest{
     ...
     * @return A string matrix with the number of jumps for each building grouyp by the lines of the building.
     */
    public boolean simulate(int[] configuration,int[][] buildings,int building)throws CityOfHeroesExcepcion{
        pairs.clear();
        cities.clear();
        solve(configuration,buildings);
        city = new CityOfHeroes(C*(int)W,600);
        cities.add(city);
        if(paths[yi][building-1].equals("X")||paths[yi][building-1].equals("0")){
            return false;
        }
        for(int i=0 ; i<C ; i++){
