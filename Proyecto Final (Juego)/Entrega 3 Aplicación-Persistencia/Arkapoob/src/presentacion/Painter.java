...
public class Painter extends PanelFondo {
	private boolean isOn,right,left,left2,right2,isRandom,puedeMover;
	...
	private Painter(PantallaJuego pantalla, boolean random) {
		...
		setBackground("resources/fondo.jpg");
	}
	public Painter(PantallaJuego pantalla, boolean random , int jugadores){
	...
	public Painter(PantallaJuego pantalla, boolean random , int jugadores,String[] nombres){
		this(pantalla,random);
		this.jugadores = jugadores;
		reiniciarTimers();
	...
	private void prepareJuego(){
		Arkanoid.nuevoArkanoid(isRandom);
		game = Arkanoid.deme(isRandom);
		try{
			game.addNivel();
			prepareJugadores();
